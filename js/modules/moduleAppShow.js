...
import Project, { ProjectFull , JoinStatus } from './moduleProject.js';
...
function init_Project() {
    ...
    mainApp.send("POST", pathDataShow , headers , formData ).then( ( result ) => {
        ...
        myProject.isMember = result["project"][0]["isMember"];
        ...
        callIsMember( myProject.isMember );
...
function clickJoin() {
    ...
    if( myProject.listMembers.length > 0 ) {
            
        let box = document.getElementById("lightBox");
        box.classList.remove("close");
        
        let msg = "لديك عدد ("+ myProject.listMembers.length +") طلبات سابقة بإنتظار الموافقة" ;
        let code = `
            <div class="divHeader">
                <h2>طلب إنضمام</h2>
            </div>
...
function requestJoin() {
    ...
    formData.append("codeUserProject", myProject.codeUser );
    formData.append("message", messageJoin );
    formData.append("codeProjectSkill", codeProjectSkill );
    
    mainApp.send( "POST" , pathJoinMemberSkill, headers, formData ).then( (result) => {
