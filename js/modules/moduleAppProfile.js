...
import User , { UserProfile , ContactType } from './moduleUserProfile.js';
import Skill , { SkillLevel } from './moduleSkill.js';
...
function init_Profile() {
    ...
    mainApp.send("POST", pathDataProfile, headers, formData ).then( (result) => {
        ...
        if(1) {
            ...
            myProfile = new UserProfile( result );
            myProfile.isFriend = isFriend;
            
            myProjectList = new ProjectList();
            myProjectList.fillListProject( result );
            
            let dataJoin = result["projectJoin"];
            let objProject = {
                project: dataJoin
            }
            myProjectListJoin = new ProjectList();
            myProjectListJoin.fillListProject( objProject );
...
function fillProfileContact() {
    ...
    const myListContact = myProfile.listContact;
...
function fillProfileCertificate() {
    ...
    const myListCertificate = myProfile.listCertificate;
...
function fillProfileExperience() {
    ...
    const myListExperience = myProfile.listExperience;
...
function fillProfileSkills() {
    ...
    const myListUserSkills = myProfile.listUserSkill;
...
function fillProjectJoin() {
    ...
    const listProjects = myProjectListJoin.getListProject();
    for(var i=0; i<listProjects.length; i++) {
...
function fillEvaluation( result ) {
    
    
    const countProject = result["countProject"];
    const countProjectShare = result["countProjectShare"];
    const countProjectShareMark = result["countProjectShareMark"];
    let avaregMark = (countProjectShareMark) ? countProjectShareMark / countProjectShare : 0;
    ...
    displayMark.textContent =  avaregMark+"/5";
...
function followUser( follow ) {
    ...
    formData.append("codeUserFollow", myProfile.code );
    
    
    mainApp.send( "POST", pathFollow , headers, formData ).then( (result) => {
        
        myProfile.isFriend = !(myProfile.isFriend);
        setButtonFollow();
...
function clickEditImage() {
    ...
    mainApp.send( "POST", pathUpdateAbout , headers, formData ).then( (result) => {
...
function clickAddContact() {
    ...
    mainApp.send( "POST", pathCreateContact , headers, formData ).then( (result) => {
...
function clickEditContact( idContact ) {
    ...
    mainApp.send( "POST", pathUpdateContact , headers, formData ).then( (result) => {
...
function clickRemoveContact( idContact ) {
    ...
    mainApp.send( "POST", pathDeleteContact , headers, formData ).then( (result) => {
...
function getIndexContactById( idContact ) {
    ...
    listContact.map( (item) => {
        ...
        {
            const indexof = listContact.indexOf(item);
            if (indexof > -1) {
                index = indexof;
            }
        }
...
function clickAddCertificate() {
    ...
    mainApp.send( "POST", pathCreateCertificate , headers, formData ).then( (result) => {
        
        myProfile.fillCertificate( result );
        fillProfileCertificate();
...
function clickEditCertificate( idCertificate ) {
    ...
    mainApp.send( "POST", pathUpdateCertificate , headers, formData ).then( (result) => {
...
function clickRemoveCertificate( idCertificate ) {
    ...
    let formData = new FormData();
    formData.append("codeUser", moduleUserLog.userCode );
    formData.append("id", idCertificate );

    mainApp.send( "POST", pathDeleteCertificate , headers, formData ).then( (result) => {
...
function getIndexCertificateById( idCertificate ) {
    
    let index = 0;
    
    listCertificate.map( (item) => {
        if(item.id == idCertificate)
        {
            const indexof = listCertificate.indexOf(item);
            if (indexof > -1) {
                index = indexof;
            }
        }
...
function clickAddExperience() {
    ...
    mainApp.send( "POST", pathCreateExperience , headers, formData ).then( (result) => {
        
        myProfile.fillExperience( result );
        fillProfileExperience();
...
function clickEditExperience( idExperience ) {
    ...
    mainApp.send( "POST", pathUpdateExperience , headers, formData ).then( (result) => {
...
function clickRemoveExperience( idExperience  ) {
    ...
    mainApp.send( "POST", pathDeleteExperience , headers, formData ).then( (result) => {
...
function getIndexExperienceById( idExperience ) {
    
    let index = 0;
    
    listExperience.map( (item) => {
        if(item.id == idExperience)
        {
            const indexof = listExperience.indexOf(item);
            if (indexof > -1) {
                index = indexof;
            }
        }
...
function clickAddSkill() {
    ...
    mainApp.send( "POST", pathCreateSkill , headers, formData ).then( (result) => {
        ...
        myProfile.fillUserSkills( result );
        fillProfileSkills();
...
function clickRemoveSkill( idSkill ) {
...
function getIndexSkillById( idSkill ) {
    ...
    listSkill.map( (item) => {
        if(item.id == idSkill)
        {
            const indexof = listSkill.indexOf(item);
            if (indexof > -1) {
                index = indexof;
            }
        }
...
function fillListData( data ) {
    ...
    for(var i=0; i<dataContactType.length; i++) {
        listContactType.push( new ContactType( dataContactType[i] ) );
    }
    ...
    for(var i=0; i<dataSkillLevel.length; i++) {
        listSkillLevel.push( new SkillLevel( dataSkillLevel[i] ) );
    }
